...
.research-header {
  ...
  @media (max-width: 768px) { padding: 0 0.75rem; margin-bottom: 3rem; }
}
...
.research-grid {
  display: grid; grid-template-columns: repeat(3, 1fr); gap: 2rem; width: 100%; max-width: 1100px;
...
.research-item {
  ...
  padding: 3rem 2rem; background: var(--card-bg); border: 1px solid var(--border);
  ...
  &:hover {
    ...
    .research-icon-circle { background: var(--color-cyan); color: #000; transform: scale(1.1) rotate(10deg); }
  }
}
...
.research-icon-circle {
  ...
  font-size: 2rem; color: var(--color-cyan); margin-bottom: 1.5rem;
  transition: all 0.4s ease;
}

.research-item-title {
  font-size: 1.1rem; font-weight: 700; color: var(--text); text-transform: uppercase; letter-spacing: 0.05em;
}
...
.impact-stats {
  display: grid; grid-template-columns: repeat(3, 1fr); gap: 3rem; width: 100%; max-width: 1100px;
  padding: 0 1.5rem;
  @media (max-width: 768px) { grid-template-columns: 1fr; gap: 2rem; padding: 0 1rem; }
}
...
.stat-number {
  font-family: var(--font-principal); font-size: 5rem; font-weight: 800;
  color: var(--color-cyan); line-height: 1; margin-bottom: 0.75rem;
  @media (max-width: 768px) { font-size: 3.5rem; }
}

.stat-label {
  font-size: 1.1rem; color: var(--text); font-weight: 600; text-transform: uppercase; letter-spacing: 0.05em;
}

.impact-footnote {
  margin-top: 3rem; font-size: 1.15rem; color: var(--text-muted);
